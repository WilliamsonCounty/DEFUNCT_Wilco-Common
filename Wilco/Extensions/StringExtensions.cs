...
public static class StringExtensions
{
    ...
    public static string ComputeHash(this FileInfo file) => file.ComputeHash("sha256sum.exe");

    public static string ComputeHash(this FileInfo file, string appPath)
    {
        ...
        var chk = p.StandardOutput.ReadToEnd();

        p.WaitForExit();
        p.Close();

        var match = Regex.Match(chk, @"(?<=\\)(.*?)(?=\ \*)", RegexOptions.IgnoreCase);
        chk = match.Success && match.Value.Length == 64 ? match.Value : string.Empty;

        return chk.ToUpper();
    }

    public static string GetExtension(this string mimeType) => mimeType.GetExtension(true);

    public static string GetExtension(this string mimeType, bool throwErrorIfNotFound)
    {
    ...
    public static string GetStringHash(this string inputString)
    {
        ...
        foreach (var b in GetHash(inputString)) _ = sb.Append(b.ToString("X2"));
